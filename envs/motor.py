...
OBSERVED_TORQUE_LIMIT = 1.6
MOTOR_VOLTAGE = 24.0
MOTOR_RESISTANCE = 0.48
MOTOR_TORQUE_CONSTANT = 0.0334
...
class MotorModel(object):
  ...
  def __init__(self, torque_control_enabled=False, kp=1.2, kd=0):
    ...
    self._current_table = [0, 4, 8, 12, 16]
    self._torque_table = [0, 0.15, 0.29, 0.5, 0.59]
    self._current_table1 = [0, 3.3, 4.2 , 5.2 , 6.3 ,9.1, 12.2, 18.1, 22.1, 31.0]
    self._torque_table1 = [0, 0.084, 0.122, 0.158, 0.212, 0.287, 0.398, 0.583, 0.712 ,0.8512]
    self._torque_table2=[i*3 for i in self._torque_table1]    
    self._torque_table_3 = [0,0.44,0.88,1.50,1.76]
  ...
  def _convert_to_torque_from_pwm(self, pwm, true_motor_velocity):
    ...
    #current=np.clip(current,-CURRENT_CLIPPING,CURRENT_CLIPPING)
    current_sign = np.sign(current)
    current_magnitude = np.absolute(current)
    #print(current_magnitude)
    # Saturate torque based on empirical current relation.
    actual_torque = np.interp(current_magnitude, self._current_table1, self._torque_table1)
    #print(actual_torque)
